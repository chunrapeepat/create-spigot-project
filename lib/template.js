...
const pluginYML = (name, version, package, author, commands) => {
  let _commands = ''
  ...
  return `name: ${name}
version: ${version}
main: ${package.join('.')}.${name}
author: ${author}
commands:
${_commands}`
}

const JavaPlugin = (name, package, commands, config) => {
  let _commands = ''
  for(let i = 0; i < commands.length; i++){
    _commands += `    getCommand("${commands[i]}").setExecutor(new ${capitalizeFirstLetter(commands[i])}Command());\n`
  }
  return `package ${package.join('.')};

import org.bukkit.Bukkit;
import org.bukkit.plugin.java.JavaPlugin;
import ${package.join('.')}.commands.*;
...
const Command = (name, package, command) => {
  return `package ${package.join('.')}.commands;

import ${package.join('.')}.${name};
